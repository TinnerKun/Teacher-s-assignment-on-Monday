...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    // check empty
    if(empty($username) || empty($password)) return header('Location: ?error=กรุณากรอกข้อมูลให้ครบ');
    ...
    if(!$user) return header('Location: ?error=ไม่พบชื่อผู้ใช้นี้');

    // check password
    if(!password_verify($password, $user->password)) return header('Location: ?error=รหัสผ่านไม่ถูกต้อง');

    // check ban user
    if($user->ban == 1) return header('Location: ?error=บัญชีผู้ใช้นี้ถูกระงับการใช้งาน');
